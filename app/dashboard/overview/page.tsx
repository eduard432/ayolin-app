	...
	return (
		...
		<main className="px-16 py-12 ">
			<div className="w-10/12 mx-auto grid grid-cols-12 gap-x-8 gap-y-4" >
				...
				<ToggleGroup value={layout} onValueChange={(value) => setLayout(value as "grid" | "list")} type="single" className="border h-full">
					<ToggleGroupItem
						value="grid"
				...
				<DropdownMenu>
					<DropdownMenuTrigger asChild>
						<Button className="h-full cursor-pointer">Add New... <ChevronDown /></Button>
					</DropdownMenuTrigger>
					<DropdownMenuContent>
			...
			<section className="col-span-8">
				...
				<div className={cn("rounded-md grid grid-cols-2", layout == "grid" ? "gap-8" : "gap-0")}>
					{proyectos.map((proyecto) => (
						<Card className={cn("rounded-none py-4", layout == "grid" ? "col-span-1 min-h-36 rounded-md" : "col-span-2")} key={proyecto.nombre}>
