...
const createChannelSchemaCommon = {
	chatbotId: z.string(),
}

const createChannelSchema = z.discriminatedUnion('keyName', [
	z
		.object({
			...createChannelSchemaCommon,
...
export const POST = auth(async (request) => {
	try {
		...
		const { chatbotId, keyName, settings } = validateWithSource(
			createChannelSchema,
			body,
			'body'
		)
